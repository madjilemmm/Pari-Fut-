...
import type { MatchPrediction, WhyMatch, FormGuide, PowerRating } from "@/lib/api";
...
function FormRow({
  ...
  label: string;
  form: { results: string[]; avg_goals_for: number | null; avg_goals_against: number | null; note?: string };
}) {
  const color = (r: string) =>
    r === "V" ? "bg-terminal-accent text-black" : r === "D" ? "bg-terminal-danger/80 text-white" : "bg-terminal-muted/40 text-white";
  return (
    <div>
...
export function MatchDetailClient({ matchId }: { matchId: string }) {
  ...
  const [why, setWhy] = useState<WhyMatch | null>(null);
  const [form, setForm] = useState<FormGuide | null>(null);
  const [power, setPower] = useState<PowerRating | null>(null);
  ...
  useEffect(() => {
    ...
    getPrediction(matchId)
      .then((p) => {
        if (cancelled) return;
        setPred(p);
        getWhy(matchId).then((w) => !cancelled && setWhy(w)).catch(() => {});
        getForm(matchId).then((f) => !cancelled && setForm(f)).catch(() => {});
        getPowerRating(matchId).then((pw) => !cancelled && setPower(pw)).catch(() => {});
      })
      .catch((e) => {
        if (!cancelled) setError(e instanceof ApiError ? e.message : "Erreur inattendue.");
      });
    return () => {
      cancelled = true;
    };
    ...
    return (
      <div className="space-y-8">
        ...
        <p className="text-center text-xs text-terminal-muted">
          Calcul du modèle en cours (walk-forward, aucune donnée future utilisée)…
        </p>
      </div>
    );
  }
          ...
          <section className="rounded-xl2 border border-terminal-border bg-terminal-panel p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <div className="font-semibold">{pct(pred.over_2_5_prob)}</div>
              </div>
              <div>
                <div className="text-terminal-muted text-xs">Moins de 2,5 buts</div>
                <div className="font-semibold">{pct(pred.under_2_5_prob)}</div>
              </div>
              <div>
                <div className="text-terminal-muted text-xs">Plus de 1,5 buts</div>
                <div className="font-semibold">{pct(pred.over_1_5_prob)}</div>
              </div>
              <div>
                <div className="text-terminal-muted text-xs">Les deux marquent</div>
